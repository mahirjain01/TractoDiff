...
class RNNDiffusion(nn.Module):
    def __init__(self, in_dim, out_dim, hidden_dim, diffusion_step_embed_dim=256, steps=1, rnn_type=RNNType.gru,
        ...
        self.steps = steps
        self.rnn_type = rnn_type
        self.output_threshold = output_threshold  # Not using here
        ...
        cond_dim = diffusion_step_embed_dim + hidden_dim # 256 + 512 = 728
    ...
    def forward(self, noisy_trajectory, time_step, global_cond, local_cond=None):
        global_feature = self.diffusion_step_encoder(time_step) # [2B x 256]
        condition = torch.concat((global_feature, global_cond), dim=-1) #[2B x 728]  
        h = self.in_fc(condition)  # [2B X 512] => Activation function matters here : think about it!!!!!

        B, N, C = noisy_trajectory.shape # [2B X 16 X 3]
        x_pre = self.waypoints(noisy_trajectory.view(B, -1)) # => self.waypoints([2B x 48]) => [2B x 512]
